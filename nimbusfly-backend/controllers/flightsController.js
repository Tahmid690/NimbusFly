...
const searchFlights = async (req, res) => {
  ...
  try {
    ...
    if (orderType === 'Cheapest') {
      order_typ = `total_ticket_price`;
    }
    else if (orderType === 'Fastest') order_typ = `flight_time`;
    else order_typ = `departure_time`
    ...
    (SELECT
        ...
        NULL as transit_airport_name,
        NULL as transit_flight_number,
        NULL as layover_time,
        FALSE as has_transit,
    ...
    (SELECT
        ...
        f2.destination_airport_id,
        a1.airline_name,
        f1.departure_time,
        f2.arrival_time,
        (f2.arrival_time - f1.departure_time) as flight_time,
        ROUND((CASE 
            WHEN $4 = 'Business' THEN (f1.business_ticket_price + f2.business_ticket_price) 
            ELSE (f1.economy_ticket_price + f2.economy_ticket_price) 
        END) * $6, 2) as total_ticket_price,
        (CASE 
            WHEN $4 = 'Business' THEN f1.business_ticket_price + f2.business_ticket_price 
            ELSE f1.economy_ticket_price + f2.economy_ticket_price 
        END) as base_price,
        LEAST(f1.baggage_limit, f2.baggage_limit) as baggage_limit,  
        ac1.model as aircraft_name,
        f1.flight_number as flight_number,  
        a1.logo_url,
        apt.iata_code as transit_airport,
        apt.airport_name as transit_airport_name,
        f2.flight_number as transit_flight_number,
        (f2.departure_time - f1.arrival_time) as layover_time,
        TRUE as has_transit,
        'connecting' as flight_type,
        2 as segment_count
    ...
    WHERE 
        ...
        AND a1.airline_id = a2.airline_id  -- Same airline constraint
        AND f1.status = TRUE AND f2.status = TRUE  -- Both flights active
        AND f1.departure_time > NOW()  -- Only future flights
        AND f2.departure_time > f1.arrival_time + INTERVAL '1 hour'  -- Minimum 1 hour layover
        AND f2.departure_time < f1.arrival_time + INTERVAL '12 hours'  -- Maximum 12 hour layover
        AND (CASE 
    ...
    SELECT
        ...
        ac1.model as aircraft_name,
        f1.baggage_limit,
        COALESCE(f1.round_trip_discount, 0) as round_trip_discount,
        ...
        COALESCE(f2.round_trip_discount, 0) as return_round_trip_discount,
        
        a1.logo_url,
        
        ROUND((CASE 
            WHEN $4 = 'Business' THEN 
                (f1.business_ticket_price * (1 - f1.round_trip_discount) + 
                 f2.business_ticket_price * (1 - f2.round_trip_discount))
            ELSE 
                (f1.economy_ticket_price * (1 - f1.round_trip_discount) + 
                 f2.economy_ticket_price * (1 - f2.round_trip_discount))
        END) * $6, 2) as total_ticket_price,
        ...
        EXTRACT(DAYS FROM (f2.departure_time - f1.arrival_time)) as days_between_flights,
    ...
    WHERE 
        apo1.iata_code = $1 
        AND apd1.iata_code = $2 
        AND DATE(f1.departure_time) = $3
        AND DATE(f2.departure_time) = $7
        AND f1.status = TRUE AND f2.status = TRUE  
        AND f1.departure_time > NOW()  
        AND f2.departure_time > NOW()  
        ...
        AND f2.departure_time >= (f1.arrival_time + INTERVAL '2 hours')  -- Minimum layover
        AND f2.departure_time <= (f1.arrival_time + INTERVAL '30 days')   -- Maximum stay duration
