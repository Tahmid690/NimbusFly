...
const getAirportCity = async (req, res) => {
  const {iata} = req.query;
  // console.log(iata);
  try {
    const result = await pool.query('SELECT city FROM airports WHERE iata_code=$1',[iata]);

    res.json({
      data: result.rows[0].city
    });
...
const searchAirportiata = async (req, res) => {
