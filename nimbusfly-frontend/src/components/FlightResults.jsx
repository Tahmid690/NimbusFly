...
const options = ['Recommended', 'Fastest', 'Cheapest'];
...
function FlightResults() {
  ...
  const [rangeprice, setrange] = useState([0, 10000]); // initial dummy values
  ...
  const fetchFlights = async () => {
    try {
      ...
      setFlights(data);
      setFilteredFlights(data); // initialize filtered results
  ...
  return (
    <div>
      ...
      <div className='grid grid-cols-7 gap-4 px-6 lg:px-8 mt-20'>
        ...
        <div className='col-span-5 p-4 h-fit'>
          <div className='flex flex-col gap-5'>
            ...
            <div className="mt-6 space-y-4">
              {filteredFlights.length === 0 ? (
                <p className="text-gray-500">No flights available for this range.</p>
              ) : (
                filteredFlights.map(flight => (
                  <div key={flight.flight_id} className="p-4 bg-white rounded-xl shadow hover:shadow-md transition">
                    <div className="flex justify-between items-center">
                      <div>
                        <h3 className="text-lg font-bold text-blue-600">{flight.airline_name}</h3>
                        <p className="text-sm text-gray-600">{flight.origin} → {flight.destination}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xl font-semibold text-blue-800">৳{flight.price}</p>
                        <p className="text-sm text-gray-500">{flight.duration} mins</p>
                      </div>
                    </div>
                  </div>
                ))
